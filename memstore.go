...
func (m *ms) Persist(ctx context.Context) error {
	...
	ins := bulk.NewInsert(ctx, MustGetDB(ctx).(*sqlx.DB),
		"hits", []string{"site", "path", "code", "ref", "ref_params", "ref_original", "ref_scheme", "created_at"})
	for _, h := range hits {
		var err error
		...
		ins.Values(h.Site, h.Path, h.Code, h.Ref, h.RefParams, h.RefOriginal, h.RefScheme, sqlDate(h.CreatedAt))
	}
	err := ins.Finish()
	if err != nil {
		zlog.Error(err)
